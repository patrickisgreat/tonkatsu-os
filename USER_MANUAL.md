...
Refresh the bundled database without network access once the curated archives
are cached locally:

1. Cache archives: place curated RRUFF zips in `data/raw/rruff/` and the pharma archive in `data/raw/pharma/` (each folder has a README with expected filenames).
2. Inspect current contents:
   ```bash
   poetry run python scripts/db_report.py
   ```
3. Import curated RRUFF spectra:
   ```bash
   poetry run python scripts/fetch_rruff_samples.py --limit 20
   ```
4. Optionally import the pharma subset:
   ```bash
   poetry run python scripts/fetch_pharma_samples.py --limit 10
   ```
5. Rebuild features and peak caches:
   ```bash
   poetry run python scripts/rebuild_features.py
   ```
6. Spot-check spectra or export to CSV:
   ```bash
   poetry run python scripts/list_spectra.py --limit 10
   ```
7. Detect duplicates (use `--prune` to drop lower-ranked entries):
   ```bash
   poetry run python scripts/find_duplicates.py
   ```

Tip: `poetry run python scripts/refresh_dataset.py` combines RRUFF + pharma
ingestion and a feature rebuild into a single command.

### Pharmaceutical Dataset

To keep the pharmaceutical reference set reproducible:
...
PY
   ```
2. Review `data/raw/pharma/samples.yaml` to adjust which compounds and how many samples to ingest.
3. Import the curated spectra:
   ```bash
   poetry run python scripts/fetch_pharma_samples.py --limit 15
   ```
