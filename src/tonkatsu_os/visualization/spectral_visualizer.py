...
class SpectralVisualizer:
    ...
    def _single_spectrum_tab(self):
        ...
        with col1:
            ...
            spectra_list = self.database.search_by_compound_name(
                selected_compound, exact_match=True
            )
    ...
    def _plot_single_spectrum_detailed(
        self,
        spectrum_data: Dict,
        show_peaks: bool,
        show_baseline: bool,
        ...
        fig.add_trace(
            go.Table(
                header=dict(values=["Metric", "Value"], fill_color="lightblue", align="center"),
                cells=dict(
    ...
    def _multi_spectrum_tab(self):
        ...
        with col1:
            ...
            overlay_type = st.radio("Display type:", ["Overlay", "Stacked", "Normalized"])

            show_peaks_multi = st.checkbox("Show all peaks", key="multi_peaks")
            show_differences = st.checkbox("Highlight differences")

            # Color scheme
            color_scheme = st.selectbox("Color scheme:", ["Default", "Scientific", "Peaks"]).lower()
        ...
        with col2:
            ...
            if all_spectra:
                self._plot_multi_spectrum_comparison(
                    all_spectra, overlay_type, show_peaks_multi, show_differences, color_scheme
                )

    def _plot_multi_spectrum_comparison(
        self,
        ...
        max_length = max(
            len(spec["preprocessed_spectrum"] or spec["spectrum_data"]) for spec in spectra_list
        )
            ...
            if show_peaks and spectrum_data["peak_positions"] is not None:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(color=colors[i % len(colors)], size=6, symbol="diamond"),
                        showlegend=False,
                    )
                )
    ...
    def _plot_analysis_results(self, results: Dict, confidence_analysis: Dict, spectrum_info: Dict):
        ...
        with col1:
            ...
            fig_confidence.update_layout(
                ...
                annotations=[
                    dict(
                        text=f"{confidence_analysis['overall_confidence']:.1%}<br>Overall",
                        x=0.5,
                        y=0.5,
                        font_size=20,
                        showarrow=False,
        ...
        model_confidences = [
            results["individual_predictions"][model]["confidence"] for model in models
        ]
        ...
        fig_models.add_shape(
            type="line",
            x0=-0.5,
            y0=results["confidence"],
            x1=len(models) - 0.5,
            y1=results["confidence"],
    ...
    def _database_explorer_tab(self):
        ...
        fig_dist.update_layout(xaxis_tickangle=-45, height=500, template="plotly_white")
        ...
        selected_compound = st.selectbox(
            "Select compound to explore:", [comp[0] for comp in stats["top_compounds"]]
        )
                    ...
                    sample_spectra.append(
                        {
                            ...
                            "data": spectrum_data["preprocessed_spectrum"]
                            or spectrum_data["spectrum_data"],
                            "id": spec_info["id"],
                        }
                    )
    ...
    def _display_comparison_statistics(self, spectra: List[np.ndarray], spectra_info: List[Dict]):
        ...
        with col1:
            st.metric(
                "Min Correlation", f"{np.min(correlation_matrix[correlation_matrix < 1]):.3f}"
            )

        with col2:
            st.metric(
                "Mean Correlation", f"{np.mean(correlation_matrix[correlation_matrix < 1]):.3f}"
            )

        with col3:
            st.metric(
                "Max Correlation", f"{np.max(correlation_matrix[correlation_matrix < 1]):.3f}"
            )
