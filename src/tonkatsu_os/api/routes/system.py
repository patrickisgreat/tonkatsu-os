...
from ..models import ApiResponse, ExportRequest, SystemHealth
...
async def health_check(db=Depends(get_database)):
    ...
    try:
        ...
        return SystemHealth(status="healthy", components={"api": True}, version=__version__)
    except Exception as e:
        logger.error(f"Health check failed: {e}")
        return SystemHealth(status="error", components={"api": False}, version=__version__)
...
async def export_database(format: str, request: ExportRequest = None, db=Depends(get_database)):
    ...
    try:
        ...
        if format == "sqlite":
            ...
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=f"_tonkatsu_export_{timestamp}.db"
            )
            ...
            # Write CSV header
            temp_file.write(
                "id,compound_name,chemical_formula,cas_number,acquisition_date,source\n"
            )
            ...
            return FileResponse(
                temp_file.name, media_type="text/csv", filename=f"tonkatsu_export_{timestamp}.csv"
            )
...
async def create_backup(db=Depends(get_database)):
    ...
    try:
        ...
        return ApiResponse(success=True, message=f"Backup created: {backup_filename}")
...
async def restart_system():
    ...
    try:
        ...
        return ApiResponse(
            success=False, message="Restart functionality not implemented in demo mode"
        )
