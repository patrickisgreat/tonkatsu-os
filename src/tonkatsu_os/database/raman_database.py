...
class RamanSpectralDatabase:
    ...
    def add_spectrum(self, 
        ...
        """, (
            compound_name, chemical_formula, cas_number,
            measurement_conditions, laser_wavelength, integration_time,
            pickle.dumps(spectrum_data), pickle.dumps(processed_spectrum),
            pickle.dumps(peaks), pickle.dumps(peak_intensities),
            pickle.dumps(fingerprint), str(metadata) if metadata else None
        ))
        ...
        """, (
            spectrum_id, pickle.dumps(feature_vector),
            pickle.dumps(peaks[:10]), spectral_hash  # Store top 10 peaks
        ))
    ...
    def search_similar_spectra(self, 
    ...
    def get_spectrum_by_id(self, spectrum_id: int) -> Optional[Dict]:
        ...
        if spectrum_dict['preprocessed_spectrum']:
            spectrum_dict['preprocessed_spectrum'] = pickle.loads(spectrum_dict['preprocessed_spectrum'])
        ...
        if spectrum_dict['spectral_fingerprint']:
            spectrum_dict['spectral_fingerprint'] = pickle.loads(spectrum_dict['spectral_fingerprint'])
    ...
    def search_by_compound_name(self, compound_name: str, exact_match: bool = False) -> List[Dict]:
        ...
        if exact_match:
            query = "SELECT id, compound_name, chemical_formula FROM spectra WHERE compound_name = ?"
            params = (compound_name,)
        else:
            query = "SELECT id, compound_name, chemical_formula FROM spectra WHERE compound_name LIKE ?"
            params = (f"%{compound_name}%",)
        
        cursor = self.conn.execute(query, params)
        return [{'id': row[0], 'compound_name': row[1], 'chemical_formula': row[2]} 
                for row in cursor.fetchall()]
    ...
    def _extract_features(self, spectrum: np.ndarray, peaks: np.ndarray, 
                         peak_intensities: np.ndarray) -> np.ndarray:
        ...
        top_peaks = peaks[:10] if len(peaks) >= 10 else np.pad(peaks, (0, 10-len(peaks)), 'constant')
        features.extend((top_peaks / len(spectrum)).tolist())
        
        return np.array(features)
    
    def _generate_fingerprint(self, spectrum: np.ndarray, peaks: np.ndarray, 
                            peak_intensities: np.ndarray) -> Dict:
        ...
        return {
            ...
            'spectral_centroid': float(np.sum(np.arange(len(spectrum)) * spectrum) / np.sum(spectrum)),
