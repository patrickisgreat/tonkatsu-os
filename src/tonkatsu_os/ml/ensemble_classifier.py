...
class EnsembleClassifier:
    ...
    def predict(self, X: np.ndarray) -> Dict:
        ...
        for i in range(len(X)):
            ...
            top_predictions = [
                {
                    'compound': self.class_names[idx],
                    'probability': ensemble_proba[i][idx]
                }
    ...
    def optimize_hyperparameters(self, X: np.ndarray, y: np.ndarray) -> Dict:
        ...
        nn_grid = GridSearchCV(
            ...
            nn_param_grid, cv=3, scoring='accuracy', n_jobs=-1
        )
...
class ConfidenceScorer:
    ...
    def calculate_confidence_score(self, 
                                  ...
                                  spectral_quality: float = 1.0) -> Dict:
        """
        ...
        overall_confidence = sum(
            weights[key] * score 
            for key, score in confidence_components.items()
        )
