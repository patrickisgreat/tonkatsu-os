...
class SpectrumImporter:
    ...
    def __init__(self, database: RamanSpectralDatabase):
        ...
        self.supported_formats = [".csv", ".txt", ".json", ".xlsx", ".tsv"]
    ...
    def _parse_csv_file(self, file) -> Dict:
        ...
        if "intensity" in df.columns.str.lower():
            ...
            spectrum_data = df[
                df.columns[df.columns.str.lower().str.contains("intensity")][0]
            ].values
    ...
    def _parse_text_data(self, content: str, format_type: str) -> np.ndarray:
        ...
        for line in data_lines:
            # Determine separator
            if format_type == "CSV (comma-separated)" or (
                format_type == "Auto-detect" and "," in line
            ):
                values = line.split(",")
    ...
    def _parse_json_spectrum(self, data: Dict) -> Dict:
        ...
        return {"spectrum_data": spectrum_data, **metadata}
...
def create_import_templates():
    ...
    csv_template = pd.DataFrame(
        {"wavenumber": np.arange(200, 2000, 2), "intensity": np.random.random(900) * 1000}
    )
