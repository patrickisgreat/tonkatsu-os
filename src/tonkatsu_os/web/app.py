...
from ml_models import ConfidenceScorer, EnsembleClassifier
...
def acquire_spectrum(port=SERIAL_PORT, baudrate=BAUD_RATE):
    ...
    try:
        with serial.Serial(port, baudrate, timeout=2) as ser:
            ...
            spectrum = np.array(
                [int(x) for x in raw_data.decode(errors="ignore").split()[:DATA_POINTS]]
            )
            return spectrum
...
def main():
    st.set_page_config(
        page_title="Tonkatsu-OS: AI-Powered Raman Analysis", page_icon="🔬", layout="wide"
    )
    ...
    st.markdown(
        "*Advanced DIY Raman spectrometer with machine learning-based molecular identification*"
    )
    ...
    if page == "🔍 Live Analysis":
        live_analysis_page(
            db, preprocessor, classifier, confidence_scorer, peak_matcher, visualizer
        )
...
def analyze_spectrum(
    raw_spectrum, db, preprocessor, classifier, confidence_scorer, peak_matcher, display_col
):
    ...
    with st.spinner("Analyzing spectrum..."):
        ...
        if os.path.exists(MODEL_PATH):
            try:
                ...
                feature_vector = preprocessor._extract_features(
                    processed_spectrum, peaks, peak_intensities
                )
        ...
        **Confidence:** {confidence:.1%} | **Agreement:** {prediction['model_agreement']:.1%}
        ...
        with st.expander("Individual Model Results"):
            for model_name, result in prediction["individual_predictions"].items():
                st.write(
                    f"**{model_name.replace('_', ' ').title()}:** {result['compound']} ({result['confidence']:.1%})"
                )
        ...
        if similar_spectra:
            ...
            for spec in similar_spectra[:3]:
                st.write(
                    f"• **{spec['compound_name']}** (similarity: {spec['similarity_score']:.1%})"
                )
    ...
    with display_col:
        ...
        if similar_spectra:
            for i, spec in enumerate(similar_spectra):
                st.write(
                    f"{i+1}. **{spec['compound_name']}** (similarity: {spec['similarity_score']:.1%})"
                )
...
def display_spectrum_analysis(spectrum_data):
    ...
    if spectrum_data:
        ...
        if len(spectrum_data["peaks"]) > 0:
            ...
            for i, (peak, intensity) in enumerate(
                zip(spectrum_data["peaks"][:5], spectrum_data["peak_intensities"][:5])
            ):
                ax2.annotate(
                    f"{peak}",
                    (peak, intensity),
        ...
        if len(spectrum_data["peaks"]) > 0:
            ...
            with col2:
                st.metric(
                    "Dominant Peak",
                    spectrum_data["peaks"][np.argmax(spectrum_data["peak_intensities"])],
                )
            with col3:
                st.metric(
                    "Peak Density",
                    f"{len(spectrum_data['peaks'])/len(spectrum_data['processed']):.3f}",
                )
    ...
    with col2:
        if st.button("Generate Synthetic Data"):
            with st.spinner("Generating synthetic data..."):
                ...
                st.success(
                    f"Generated {results.get('successful_integrations', 0)} synthetic spectra"
                )
...
def model_training_page(db, classifier):
    ...
    st.info(
        f"Database contains {stats['total_spectra']} spectra from {stats['unique_compounds']} compounds"
    )
...
def settings_page():
    ...
    with col2:
        data_points = st.number_input(
            "Data Points", value=DATA_POINTS, min_value=512, max_value=4096
        )
    ...
    info_data = {
        "Database Size": f"{Path(DATABASE_PATH).stat().st_size / 1024 / 1024:.1f} MB"
